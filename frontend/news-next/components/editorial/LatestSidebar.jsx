      ...
      <div className="space-y-5">
        {posts.slice(0, 5).map((post, index) => (
          <article
            ...
            className={`news-latest-item ${index > 0 ? "border-t border-slate-200 pt-5" : ""}`.trim()}
          >
            <div className="flex items-start gap-3">
              <div className="min-w-0 flex-1">
                <Link href={`/berita/${post.slug}`} className="block">
                  <h3 className="text-sm font-semibold leading-snug text-slate-900 transition-colors hover:text-sky-700">
                    {post.title}
                  </h3>
                </Link>
              ...
              <Link
                ...
                className="relative h-[60px] w-[80px] shrink-0 overflow-hidden rounded-[2px] bg-slate-100 shadow-sm"
              >
                <Image
