...
function pickFeaturedCategory(categories) {
  return (
    categories.find((category) =>
      ["politics", "politik"].includes(category.slug),
    ) ??
    categories[0] ??
    null
  );
    ...
    const categoryPostMap = Object.fromEntries(
      await Promise.all(
        visibleCategories.map(async (category) => [
          category.id,
          await getPostsByCategory(category.id, 8),
        ]),
      ),
    );

    const featuredCategory = pickFeaturedCategory(categories);
    const featuredCategoryPosts = featuredCategory
      ? await getPostsByCategory(featuredCategory.id, 8)
      : [];
...
export default async function EditorialNewsPage() {
      ...
      <main className="editorial-container py-16">
        <div className="mx-auto max-w-2xl rounded-2xl border border-amber-200 bg-amber-50 p-8 text-amber-900">
          ...
          <p className="mt-3 text-sm leading-6">
            Isi <span className="font-semibold">WP_API_URL</span> di file
            .env.local untuk menampilkan data berita dari WordPress REST API.
          </p>
  ...
  return (
    <main className="bg-[#f4f4f4]">
      ...
      <div className="editorial-container space-y-12 py-8 lg:py-10">
