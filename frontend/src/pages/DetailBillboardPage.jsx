...
function DetailBillboardPage() {
    ...
    return (
        <main className="mbb-panel">
            <style>{`
        .mbb-panel{width:100%;min-height:100vh;background:#09090b;position:relative;font-family:'Inter',system-ui,-apple-system,sans-serif;}
        .mbb-close{position:fixed;top:24px;right:24px;z-index:999;width:44px;height:44px;border-radius:100px;border:1px solid rgba(255,255,255,.15);background:rgba(0,0,0,.4);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);box-shadow:0 8px 32px rgba(0,0,0,.15);color:#fff;display:grid;place-items:center;cursor:pointer;transition:transform .2s ease,background .2s ease}
        .mbb-close:hover{transform:scale(1.1);background:rgba(0,0,0,.6)}
        .mbb-sec{position:relative}
        .mbb-hero{min-height:90vh;padding:120px 8% 80px;background:#09090b;display:grid;align-items:center}
        .mbb-hero-grid{display:grid;grid-template-columns:1.05fr .95fr;gap:48px;align-items:center}
        .mbb-pill{margin:0;width:fit-content;padding:8px 16px;border-radius:100px;border:1px solid rgba(255,255,255,.15);background:rgba(255,255,255,.04);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);color:rgba(255,255,255,0.9);font:600 12px/1 'Inter',sans-serif;letter-spacing:.08em}
        .mbb-heading{margin:24px 0 16px;color:#fff;font:800 clamp(38px,4.8vw,68px)/1.1 'Plus Jakarta Sans',sans-serif;letter-spacing:-0.02em}
        .mbb-heading span{display:inline-block}
        .mbb-sub{margin:0;color:rgba(255,255,255,0.8);font:500 clamp(16px,1.5vw,22px)/1.5 'Inter',sans-serif}
        .mbb-desc{margin:12px 0 0;color:rgba(255,255,255,0.5);font:400 16px/1.75 'Inter',sans-serif;max-width:600px}
        .mbb-stats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px;margin-top:40px;margin-bottom:40px;}
        .mbb-stat{border-radius:16px;padding:20px;background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.08);transition:all 0.2s ease;}
        .mbb-stat:hover{background:rgba(255,255,255,.04);transform:translateY(-2px);}
        .mbb-stat h4{margin:0;color:#0071e3;font:800 36px/1 'Plus Jakarta Sans',sans-serif}
        .mbb-stat p{margin:8px 0 0;color:rgba(255,255,255,.5);font:500 13px/1.4 'Inter',sans-serif}
        ...
        .mbb-btn.ghost{background:transparent;color:#fff;border-color:rgba(255,255,255,.15)}
        .mbb-btn.ghost:hover{background:rgba(255,255,255,.05);transform:scale(1.02)}
        .mbb-media{position:relative;border-radius:24px;overflow:hidden;min-height:560px;box-shadow:0 24px 64px rgba(0,0,0,.4)}
        .mbb-media img{width:100%;height:100%;object-fit:cover;display:block}
        .mbb-float{position:absolute;top:20px;right:20px;padding:10px 16px;border-radius:100px;background:rgba(30,30,35,.6);border:1px solid rgba(255,255,255,.12);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);color:#fff;font:600 12px/1 'Inter',sans-serif;letter-spacing:.04em;box-shadow:0 8px 32px rgba(0,0,0,.2)}
        .mbb-marquee{height:52px;background:rgba(255,255,255,0.02);border-top:1px solid rgba(255,255,255,0.05);border-bottom:1px solid rgba(255,255,255,0.05);overflow:hidden;display:flex;align-items:center}
        .mbb-marquee-track{white-space:nowrap;display:flex;animation:mbbMarquee 30s linear infinite;color:rgba(255,255,255,0.4);font:500 13px/1 'Inter',sans-serif;letter-spacing:.1em}
        .mbb-marquee-track span{padding-right:60px}
        .mbb-light{background:#fafafa;padding:120px 8%}
        .mbb-kicker{margin:0;color:rgba(0,0,0,0.4);font:600 11px/1 'Inter',sans-serif;letter-spacing:.14em;text-transform:uppercase}
        .mbb-title{margin:12px 0 0;color:#09090b;font:800 clamp(32px,4.5vw,56px)/1.1 'Plus Jakarta Sans',sans-serif;letter-spacing:-0.02em}
        .mbb-grid3{margin-top:40px;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:24px}
        .mbb-card{background:#fff;border:1px solid #e5e7eb;border-radius:16px;padding:32px;transition:all .2s ease}
        .mbb-card:hover{transform:scale(1.01);border-color:#d1d5db;box-shadow:0 12px 32px rgba(0,0,0,.04)}
        .mbb-card span{color:#0071e3;font:500 13px/1 'ui-monospace',SFMono-Regular,Menlo,Monaco,Consolas,monospace}
        .mbb-card h4{margin:16px 0 12px;color:#09090b;font:700 18px/1.4 'Plus Jakarta Sans',sans-serif;letter-spacing:-0.01em}
        .mbb-card p{margin:0;color:rgba(0,0,0,.55);font:400 15px/1.65 'Inter',sans-serif}
        .mbb-flow{background:#f8f9fa;padding:120px 8%;position:relative;}
        .mbb-flow .mbb-kicker{color:rgba(0,0,0,0.4)}
        .mbb-flow .mbb-title{color:#09090b}
        ...
        .mbb-step-dot{width:48px;height:48px;border-radius:100px;margin:0 auto 16px;background:rgba(255,255,255,.8);box-shadow:0 8px 24px rgba(0,0,0,.04);border:1px solid rgba(0,0,0,.05);color:#0ea5e9;display:grid;place-items:center;font:700 15px/1 'Inter',sans-serif;transition:all .2s ease;backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}
        ...
        .mbb-step h5{margin:0 0 10px;color:#09090b;font:700 16px/1.4 'Plus Jakarta Sans',sans-serif;letter-spacing:-0.01em}
        .mbb-step p{margin:0;color:rgba(0,0,0,.5);font:400 14px/1.55 'Inter',sans-serif}
        .mbb-reach{background:#ffffff;padding:120px 8%;border-top:1px solid #f3f4f6}
        .mbb-reach-grid{margin-top:40px;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px}
        .mbb-reach-card{text-align:center;border-right:1px solid #f3f4f6;padding:20px 0}
        .mbb-reach-card:last-child{border-right:none}
        .mbb-reach-card h4{margin:0;color:#09090b;font:800 clamp(48px,5.5vw,72px)/1 'Plus Jakarta Sans',sans-serif;letter-spacing:-0.02em}
        .mbb-reach-card p{margin:12px 0 0;color:rgba(0,0,0,.5);font:500 14px/1.4 'Inter',sans-serif;text-transform:uppercase;letter-spacing:0.04em}
        .mbb-cta{background:#09090b;padding:120px 8%;position:relative;overflow:hidden;display:grid;place-items:center;}
        .mbb-cta::before{content:'';position:absolute;width:40vw;height:40vw;border-radius:50%;background:#0071e3;filter:blur(80px);opacity:0.1;pointer-events:none;z-index:0;}
        .mbb-cta-inner{position:relative;z-index:1;max-width:700px;margin:0 auto;text-align:center;display:grid;gap:20px;padding:64px 40px;border-radius:32px;background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.05);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);box-shadow:0 24px 64px rgba(0,0,0,.2)}
        .mbb-cta h3{margin:0;color:#fff;font:800 clamp(32px,4.5vw,48px)/1.1 'Plus Jakarta Sans',sans-serif;letter-spacing:-0.02em}
        .mbb-cta p{margin:0;color:rgba(255,255,255,.6);font:400 18px/1.6 'Inter',sans-serif}
        ...
        .mbb-cta .mbb-btn.outline{background:rgba(255,255,255,.05);color:#fff;border-color:rgba(255,255,255,.15)}
        .mbb-cta .mbb-btn.outline:hover{background:rgba(255,255,255,.1);transform:scale(1.02)}
        ...
        @media (max-width:1023px){
            ...
            .mbb-reach-card{border-right:none;border-bottom:1px solid #f3f4f6;padding:32px 0}
